...
typedef struct MS5837_values_t{
	int32_t TEMP;
	int32_t P;
	uint16_t C[8];
	uint32_t D1;
...
static float depthVal, altitudeVal;

void bar30_init(){
	HAL_StatusTypeDef ret;
	ret = MS5837_init(&hi2c2);
	if(ret != HAL_OK) Error_Handler();
}


/*
void read_bar30(void *param) {

   MS5837_init(&hi2c2);
	while(1) {
		MS5837_read(&hi2c2);
		depthVal = depth();
		altitudeVal = altitude();
		osDelay(10);
	}
}*/
